// src/components/AddDinasLuar.tsx
"use client";
...
export function AddDinasLuar() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, "dinasLuar"), {
        NIP: nip,
        Nama: nama,
        Jabatan: jabatan,
        UnitKerja: unitKerja,
      ...
      console.log("Data dinas luar berhasil ditambahkan!");
      setOpen(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button>Tambah Data Dinas Luar</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>Tambah Data Dinas Luar</DialogTitle>
            <DialogDescription>
              Isi semua detail yang diperlukan di bawah ini.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <Input placeholder="Link Dokumen" value={dokumen} onChange={(e) => setDokumen(e.target.value)} />
