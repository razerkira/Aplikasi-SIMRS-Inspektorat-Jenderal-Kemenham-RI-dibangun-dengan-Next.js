// src/components/RejectDialog.tsx
"use client";
...
export default function RejectDialog({ submissionId, tableName, refreshData }: RejectDialogProps) {
  ...
  const handleReject = async () => {
    if (!reason.trim()) {
      toast({ variant: "destructive", title: "Gagal", description: "Alasan penolakan tidak boleh kosong." });
      return;
    }
    ...
    try {
      const { error } = await supabase
        .from(tableName)
        .update({
          status_verifikator: 'Ditolak',
          alasan_penolakan_verifikator: reason,
        })
        .eq('id', submissionId);
