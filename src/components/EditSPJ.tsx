...
import { SPJ } from "./SPJTable";

export function EditSPJ({ data, refreshData }: { data: SPJ, refreshData: () => void }) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Sukses!",
        description: "Data SPJ telah berhasil diperbarui.",
      });
    } catch (error: any) {
      console.error("Error updating SPJ document: ", error.message);
      toast({
        variant: "destructive",
        title: "Gagal!",
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px]">
        <form onSubmit={handleUpdate}>
          <DialogHeader>
            <DialogTitle>Edit Data SPJ</DialogTitle>
          </DialogHeader>
